...
def create_volume_claim(ctx, name, disk):
    ctx.run(
        'helm install --name {name} bucket/gce-persistent-volume --set volumeName={name},gcePersistentDiskName={disk},claimName={name}')
...
def install(ctx, config):
    ...
    ctx.run('helm repo add bucket https://bucket.github.io/charts', echo=True)
...
def confirm(prompt='Continue?\n', failure_prompt='User cancelled task'):
    ...
    try:
        ...
        print('Unknown Response. Confirm with y, yes, t, true, on or 1; cancel with n, no, f, false, off or 0.')
